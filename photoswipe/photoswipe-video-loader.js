/* https://codepen.io/Harat/pen/jOEzJmP */

var initVideoSwipeFromDOM = function(gallerySelector) {
  ...
  var parseThumbnailElements = function(el) {
    ...
    for (var i = 0; i < numNodes; i++) {
      ...
      if (figureEl.children.length > 1) {
        // <figcaption> content
        item.title = jQuery(figureEl).find('figcaption').html();
      }
...
jQuery(document).ready(function() {
    jQuery('.video-link-gallery .photoswipe-wrapper').each(function() {
        jQuery(this).find('figure 	a').each(function() {
            jQuery(this).attr('data-size', jQuery(this).find('img').get(0).naturalWidth + 'x' + jQuery(this).find('img').get(0).naturalHeight);
        });
    });

    initVideoSwipeFromDOM('.video-link-gallery .photoswipe-wrapper');
});
